    ...
    internal class Blp2Parser : ImageParser
    {
        ...
        private void ParseInternal(Stream stream)
        {
            using (BinaryReader br = new BinaryReader(stream))
            {
                ...
                {
                    if (paletteBits != 8)
                        throw new ArgumentException("Palettized data can only have 8 bits per pixel.", "data");
                }
        ...
        private unsafe Image FromBinary(int width, int height, int[] bmpData)
        {
            ...
            if (m_alphaBits > 0)
            {
                ...
                {
                    uint nVal = unchecked((uint)((( (uint)bmpData[i * 4] << 16) & 0xff0000) | (( (uint)bmpData[i * 4 + 1] << 8) & 0xff00) | (( (uint)bmpData[i * 4 + 2]) & 0xff) | (( (uint)bmpData[i * 4 + 3] & 0x000000ff) << 24) & 0xffffffff));
            ...
            else
            {
                for (int i = 0; i < width * height; i++)
                {
                    uint nVal = unchecked((uint)((( (uint)bmpData[i * 3] << 16) & 0xff0000) | (( (uint)bmpData[i * 3 + 1] << 8) & 0xff00) | (( (uint)bmpData[i * 3 + 2]) & 0xff) | 0xff000000) & 0xffffffff);
                    *pBits++ = nVal;
                }
            }
